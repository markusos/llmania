...
class WorldGenerator:
    def generate_map(
        ...
        # Ensure win_pos != player_start_pos, unless map is too small.
        if not (
            width == 1 and height == 1
        ):  # Only try to find a new win_pos if map is larger than 1x1
            attempts = 0  # Counter to prevent infinite loops on small maps
            max_attempts = (width * height) * 2  # Heuristic for max attempts

            while win_pos == player_start_pos:
                if attempts >= max_attempts:
                    # Could not find a different win_pos.
                    # Might happen on very small maps with unlucky random choices.
