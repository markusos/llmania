...
from src.tile import Tile # Added Tile import

class WorldGenerator:
    def generate_map(self, width: int, height: int, seed: int | None = None) -> tuple[WorldMap, tuple[int, int], tuple[int, int]]:
        # 1. Initialization
        if seed is not None:
            random.seed(seed)
        ...
        while win_pos == player_start_pos: # Ensure win_pos is different
            if width < 3:
                win_x = 0 if width == 1 else random.randint(0, width -1)
            else:
                win_x = random.randint(1, width - 2)
        ...
        all_floor_tiles_for_walk_starts = [player_start_pos] # Start one walk from player
        ...
        for walk_start_x, walk_start_y in all_floor_tiles_for_walk_starts:
            ...
            for _ in range(walk_length):
                ...
                if 0 <= next_x < width and 0 <= next_y < height:
                    ...
                    current_x, current_y = next_x, next_y # Walker moves regardless of tile type
        ...
        if not floor_tiles:
            ...
            if win_pos not in floor_tiles:
                 world_map.set_tile_type(win_pos[0], win_pos[1], "floor") # Ensure it's floor
                 if win_pos not in floor_tiles: floor_tiles.append(win_pos)


        goal_item = Item("Amulet of Yendor", "The object of your quest!", {"type": "quest"})
        ...
        if world_map.get_tile(win_pos[0], win_pos[1]).type == "floor":
            ...
            if floor_tiles:
                ...
                win_pos = fallback_win_pos # Update win_pos to where item is actually placed
                world_map.place_item(goal_item, win_pos[0], win_pos[1])
        ...
        for _ in range(num_placements):
            ...
            if tile_obj and tile_obj.item is None and tile_obj.monster is None and \
               chosen_tile_pos != player_start_pos and chosen_tile_pos != win_pos:
