...
def main_debug():
    ...
    map_representation = game.render_map(debug_render_to_list=True) # Ensure this line is present in main_debug
    ...
    if map_with_look_messages:
        for row in map_with_look_messages:
            ...
            if row.startswith("You are at") or row.startswith("You see a") or \
               row.startswith("There is a") or row.startswith("The area is clear."):
                print(row)
            elif "HP:" in row or "MODE:" in row: # print HP and mode lines as well for context
                 print(row)
    ...
    if map_after_take_attempt:
        ...
        for row in map_after_take_attempt:
            if not (row.startswith("🧱") or row.startswith("🟩") or row.startswith("🧑") or \
                    row.startswith("👹") or row.startswith("💰") or row.startswith("?") or \
                    "HP:" in row or "MODE:" in row or ">" in row or \
                    row.startswith("You move east.") # Exclude map and previous messages
                  ):
                print(row)
...
if __name__ == "__main__":
    if "--debug" in sys.argv:
        ...
        game = GameEngine(map_width=30, map_height=15, debug_mode=False) # Ensure debug_mode is False
        try:
            game.run()
