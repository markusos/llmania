...
def setup_world():
    ...
    mock_connectivity_manager.get_reachable_floor_tiles = MagicMock(return_value=set([(1,1), original_win_pos])) # Basic reachability
...
def test_adjust_density_increase_floor_tiles(setup_world):
    ...
    adjuster.adjust_density(world_map, player_start_pos, original_win_pos, world_map.width, world_map.height, target_portion)
    ...
    assert abs(final_floor_count - expected_floor_tiles) <= total_inner_tiles * 0.1 or final_floor_count >= expected_floor_tiles, \
...
def test_adjust_density_decrease_floor_tiles(setup_world):
    ...
    target_portion = 0.2
    expected_floor_tiles = int(total_inner_tiles * target_portion) # 0.2 * 64 = 12.8 -> 12
    ...
    assert final_floor_count >= expected_floor_tiles - (total_inner_tiles * 0.05) and \
           final_floor_count <= expected_floor_tiles + (total_inner_tiles * 0.15), \
           f"Final floor count {final_floor_count} not close enough to target {expected_floor_tiles}. Initial was {initial_floor_count}"
...
def test_adjust_density_with_protected_coords(setup_world):
    ...
    adjuster.adjust_density(world_map, player_start_pos, original_win_pos, world_map.width, world_map.height, 0.5, protected_coords=protected)

    for x, y in protected:
        tile = world_map.get_tile(x, y)
        assert tile is not None
        assert tile.type == "wall", f"Protected coordinate ({x},{y}) was modified from wall."
...
def test_density_adjustment_respects_minimal_connectivity(setup_world):
    ...
    adjuster.adjust_density(small_map, player_s, win_s, small_width, small_height, 0.1) # Target very few floors

    for x, y in path_coords:
        tile = small_map.get_tile(x, y)
        assert tile is not None and tile.type == "floor", f"Path tile ({x},{y}) was changed from floor to {tile.type if tile else 'None'}." # type: ignore
    ...
    assert final_floors >= len(path_coords), f"Final floor count {final_floors} is less than the minimal path length {len(path_coords)}."
