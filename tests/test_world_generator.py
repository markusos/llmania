...
def test_generate_map_content(generator):
    width, height = 20, 20
    world_map, player_start_pos, win_pos = generator.generate_map(width, height, seed=123) # Use seed for consistency
    ...
    assert player_tile.type == "floor", f"Player start tile at {player_start_pos} is not a floor tile."
    ...
    assert win_tile.item.name == "Amulet of Yendor", "Win tile does not contain the Amulet of Yendor."
    assert win_tile.item.properties.get("type") == "quest", "Goal item is not of type 'quest'."
    ...
    for y_coord in range(height):
        for x_coord in range(width):
            ...
            if tile:
                if tile.item and tile.item.name != "Amulet of Yendor": # Exclude goal item
                    other_items_count += 1
                if tile.monster:
                    monsters_count += 1
...
def test_generate_map_reproducibility_with_seed(generator):
    width, height, seed = 20, 20, 42 # Use a slightly larger map for better test of content
    ...
    for y in range(height):
        for x in range(width):
            ...
            assert tile1.type == tile2.type, f"Tile type at ({x},{y}) differs: M1={tile1.type}, M2={tile2.type} with seed {seed}."
            
            # Compare items
            if tile1.item:
                assert tile2.item is not None, f"Tile1 has item '{tile1.item.name}' at ({x},{y}), Tile2 does not. Seed {seed}."
                assert tile1.item.name == tile2.item.name, f"Item names at ({x},{y}) differ: M1='{tile1.item.name}', M2='{tile2.item.name}'. Seed {seed}."
                assert tile1.item.description == tile2.item.description, f"Item descriptions at ({x},{y}) for '{tile1.item.name}' differ. Seed {seed}."
                assert tile1.item.properties == tile2.item.properties, f"Item properties at ({x},{y}) for '{tile1.item.name}' differ. Seed {seed}."
            else:
                assert tile2.item is None, f"Tile1 has no item at ({x},{y}), Tile2 has '{tile2.item.name}'. Seed {seed}."

            # Compare monsters
            if tile1.monster:
                assert tile2.monster is not None, f"Tile1 has monster '{tile1.monster.name}' at ({x},{y}), Tile2 does not. Seed {seed}."
                assert tile1.monster.name == tile2.monster.name, f"Monster names at ({x},{y}) differ: M1='{tile1.monster.name}', M2='{tile2.monster.name}'. Seed {seed}."
                assert tile1.monster.health == tile2.monster.health, f"Monster health at ({x},{y}) for '{tile1.monster.name}' differs. Seed {seed}."
                assert tile1.monster.attack_power == tile2.monster.attack_power, f"Monster AP at ({x},{y}) for '{tile1.monster.name}' differs. Seed {seed}."
                ...
                assert tile2.monster is None, f"Tile1 has no monster at ({x},{y}), Tile2 has '{tile2.monster.name}'. Seed {seed}."
...
def test_generate_map_with_different_seeds(generator):
    ...
    map1_repr = "\n".join(["".join([map1.get_tile(x,y).type[0] for x in range(width)]) for y in range(height)])
    map2_repr = "\n".join(["".join([map2.get_tile(x,y).type[0] for x in range(width)]) for y in range(height)])

    # Also check item/monster placements by counting them.
    items1, monsters1 = 0, 0
    for y in range(height):
        for x in range(width):
            if map1.get_tile(x,y).item: items1 +=1
            if map1.get_tile(x,y).monster: monsters1 +=1
            
    items2, monsters2 = 0, 0
    for y in range(height):
        for x in range(width):
            if map2.get_tile(x,y).item: items2 +=1
            if map2.get_tile(x,y).monster: monsters2 +=1

    are_maps_different = (ps1 != ps2 or 
                          wp1 != wp2 or 
                          map1_repr != map2_repr or
                          items1 != items2 or
                          monsters1 != monsters2)
                
    assert are_maps_different, "Generated maps with different seeds were identical, which is highly unlikely."


# BFS helper function for path verification
from collections import deque

def find_path_bfs(world_map: WorldMap, start_pos: tuple[int, int], end_pos: tuple[int, int]) -> bool:
    ...
    while queue:
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]: # S, N, E, W (order doesn't matter for BFS path existence)
            ...
            if 0 <= next_x < width and 0 <= next_y < height:
                ...
                if tile and tile.type == "floor" and (next_x, next_y) not in visited:
                    ...
                    queue.append(((next_x, next_y), path + [(next_x, next_y)])) # path list not strictly needed for existence check
    return False # No path

# Test for guaranteed path
@pytest.mark.parametrize("seed_val", [None] + list(range(5))) # Test with random map and a few seeded maps
def test_guaranteed_path_exists(generator, seed_val):
    ...
    width, height = 10, 10 
    if width <3 or height <3: # Skip test if dimensions are too small for edge avoidance logic
        pytest.skip("Map dimensions too small for current generator's edge avoidance logic in start/win pos.")
    ...
    assert path_found, f"No path found between player_start {player_start} and win_pos {win_pos}. Seed: {seed_val}"
...
def test_start_win_positions_not_on_edge(generator):
    ...
    for seed_val in range(5):
        ...
        assert 0 < player_start[0] < width - 1, f"Player start X ({player_start[0]}) is on edge. Seed: {seed_val}"
        assert 0 < player_start[1] < height - 1, f"Player start Y ({player_start[1]}) is on edge. Seed: {seed_val}"
        assert 0 < win_pos[0] < width - 1, f"Win pos X ({win_pos[0]}) is on edge. Seed: {seed_val}"
        assert 0 < win_pos[1] < height - 1, f"Win pos Y ({win_pos[1]}) is on edge. Seed: {seed_val}"
...
def test_start_win_positions_small_maps(generator):
    ...
    for seed_val in range(3):
        ...
        # Path test for 1x1
        world_map_1x1, ps_1x1, wp_1x1 = generator.generate_map(width, height, seed=seed_val)
