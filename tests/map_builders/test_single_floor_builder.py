...
def single_floor_builder_factory():
    def _factory(width: int, height: int, seed: int = None, floor_portion: float = 0.5, existing_map: WorldMap = None):
        return SingleFloorBuilder(width, height, seed=seed, floor_portion=floor_portion, existing_map=existing_map)
    return _factory
...
def test_single_floor_start_poi_positions_not_on_edge(single_floor_builder_factory):
    for seed_val in range(5): # Test a few seeds
        ...
        assert 0 < player_start[0] < width - 1, f"Player start X on edge: {player_start} (seed {seed_val})"
        assert 0 < player_start[1] < height - 1, f"Player start Y on edge: {player_start} (seed {seed_val})"
        assert 0 < poi_pos[0] < width - 1, f"POI X on edge: {poi_pos} (seed {seed_val})"
        assert 0 < poi_pos[1] < height - 1, f"POI Y on edge: {poi_pos} (seed {seed_val})"
        assert player_start != poi_pos, f"Player start and POI are same: {player_start} (seed {seed_val})"
...
def test_single_floor_builder_valid_minimum_size(single_floor_builder_factory):
    ...
    for width, height in valid_sizes:
        try:
            ...
            pytest.fail(f"SingleFloorBuilder raised ValueError for valid size {width}x{height}")
...
def test_single_floor_builder_invalid_small_size(single_floor_builder_factory):
    ...
    for width, height in invalid_sizes:
        with pytest.raises(ValueError, match=r"Map too small for single floor generation. Minimum size is 10x10"):
            builder = single_floor_builder_factory(width, height, seed=1)
            builder.build()
...
def test_single_floor_outer_layer_is_always_wall(single_floor_builder_factory):
    ...
    for width, height in sizes_to_test:
        ...
        world_map, _, _ = builder.build()
        for x in range(width):
            assert world_map.get_tile(x, 0).type == "wall", f"Top edge not wall at ({x},0) for size {width}x{height}" #type: ignore
            assert world_map.get_tile(x, height - 1).type == "wall", f"Bottom edge not wall at ({x},{height-1}) for size {width}x{height}" #type: ignore
        for y in range(1, height - 1): # Exclude corners already checked
            assert world_map.get_tile(0, y).type == "wall", f"Left edge not wall at (0,{y}) for size {width}x{height}" #type: ignore
            assert world_map.get_tile(width - 1, y).type == "wall", f"Right edge not wall at ({width-1},{y}) for size {width}x{height}" #type: ignore
...
def test_single_floor_all_floor_tiles_are_accessible(single_floor_builder_factory):
    ...
    if not floor_tiles: # If somehow no floor tiles were generated (should not happen with valid size)
    ...
    reachable_from_start = connectivity_manager.get_reachable_floor_tiles(world_map, [player_start_pos], width, height)
...
def test_single_floor_floor_portion_respected(single_floor_builder_factory):
    ...
    for width, height in sizes:
        ...
        for portion in portions_to_test:
            builder = single_floor_builder_factory(width, height, seed=1, floor_portion=portion)
            world_map, _, _ = builder.build()
            ...
            actual_portion = num_floor_tiles / total_inner_tiles if total_inner_tiles > 0 else 0
