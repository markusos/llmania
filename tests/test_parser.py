...
def test_parse_unrecognized_command(parser):
    ...
    assert parser.parse_command("n north") is None # "n" with argument "north" is not explicitly defined as valid
...
@pytest.mark.parametrize("command_input, expected_output", [
...
@pytest.mark.parametrize("command_input, expected_output", [
    ...
    ("  use   scroll   of   teleportation  ", ("use", "scroll of teleportation")), # Extra spaces + multiword
])
...
@pytest.mark.parametrize("command_input, expected_output", [
    ...
    ("ATTACK Orc Leader", ("attack", "orc leader")), # Case insensitivity + multiword
    ("  fight   giant   spider  ", ("attack", "giant spider")), # Extra spaces + multiword
])
