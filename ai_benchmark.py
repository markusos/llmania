...
def run_game_with_timeout(
    seed: int, timeout_seconds: int = 1, log_limit: int = 20
    ...
    try:
        ...
        if error:
            if seed < log_limit:
                with open(os.path.join(CRASH_LOG_DIR, f"crash_seed_{seed}.log"), "w") as f:
                    f.write("--- STDOUT ---\n")
                    f.write(output)
                    f.write("\n--- STDERR ---\n")
                    f.write(error)
        ...
        if seed < log_limit:
            output = e.stdout.decode("utf-8", "ignore") if e.stdout else "No output captured before timeout."
            with open(os.path.join(LOG_DIR, f"timeout_seed_{seed}.log"), "w") as f:
                f.write(output)
        return "timeout", seed
